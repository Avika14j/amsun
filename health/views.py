...
# def patient_dashboard1(request):
...
#     return render(request,'patient/home.html',d)
...
def diagnosis_dashboard(request):
    return render(request,'diagnosis/diagnosis_dashboard.html')
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        user = User.objects.get(username=request.user)
                        error = Diagnosis.objects.get(user=user)
                        return redirect('diagnosis_dashboard')
                    except:
                        try:
                            user = User.objects.get(username=request.user)
                            if user.is_staff:
...
def Registeration(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if m == "Diagnosis":
                Diagnosis.objects.create(user=user,status="pending")
...
def Diagnosis_Change_Password(request):
    if request.method=="POST":
        n = request.POST['pwd1']
        c = request.POST['pwd2']
        d = request.POST['pwd3']
        if c == d:
            ...
            u.save()
            messages.success(request,'Password Changed Successfully')
            return redirect("diagnosis_change_password")
...
def Diagnosis_Profile(request):
    user = User.objects.get(id=request.user.id)
    pat = Diagnosis.objects.get(user=user)
    form = DiagnosisForm(request.POST or None,instance=pat)
    if request.method == "POST":
        form = DiagnosisForm(request.POST or None,request.FILES or None, instance=pat)
        if form.is_valid():
            form.save()
            user.first_name = request.POST['first_name']
            user.last_name = request.POST['last_name']
            ...
            user.save()
            messages.success(request,'Profile Updated Successfully')
            return redirect("diagnosis_profile")
    d = {'doc':pat,'form':form}
    return render(request,'diagnosis/profile.html',d)
...
def admin_view_diagnosis(request):
    data=Diagnosis.objects.all()
    d={'data':data}
    return render(request,'admin/admin_view_diagnosis.html',d)
...
def diagnosis_patient_search_by_id(request):
    # if not access(request.user):
    #     messages.success(request,'Update Your Profile and Wait for Verification')
    #     return redirect('diagnosis_profile')
    data=""
    ...
    if request.method=="POST":
        ...
        try:
            ...
            messages.success(request,'Invalid Card Number')
    d={'data':data,'i':i,'appointment':appointment,'hos_appointment':hos_appointment}
    return render(request,'diagnosis/patient_search_by_id.html',d)
...
def diagnosis_status(request,pid):
    pat = Diagnosis.objects.get(id=pid)
    if pat.status=="pending":
        pat.status = "accept"
        pat.save()
        messages.success(request,'Selected Diagnosis granted to Permission')
    else:
        pat.status = "pending"
        pat.save()
        messages.success(request, 'Selected Diagnosis Withdraw to Permission')
    return redirect('admin_view_diagnosis')
...
def need_help(request):
    if request.method == "POST":
        ...
        bn = request.POST['bed_number']
        desc = request.POST['description']
        hospital = Hospital.objects.get(id=h)
        patient = Patient.objects.get(user=request.user)
        help_obj = Help_Record.objects.create(patient=patient, bed_number=bn, description=desc, hospital=hospital)
        messages.success(request,'We have sent your help request to our organisation successfully.')
        return redirect('view_need_help')
    hos_obj = Hospital.objects.all()
    d = {'hospital':hos_obj}
    return render(request, 'patient/need_help.html',d)

def view_need_help(request):
    user = Patient.objects.get(user=request.user)
    help_obj = Help_Record.objects.filter(patient=user)
    d = {'help':help_obj}
    return render(request, 'patient/view_need_help.html',d)

def help_record(request):
    user = Doctor.objects.get(user=request.user)
    help_obj = Help_Record.objects.filter(helper_doctor=user)
    d = {'help':help_obj}
    return render(request, 'doctor/help_record.html',d)

def new_need_help(request):
    help_obj = Help_Record.objects.filter(status='Pending')
    d = {'help':help_obj}
    return render(request, 'doctor/new_need_help.html',d)

def delete_need_help(request, pid):
    help_obj = Help_Record.objects.get(id=pid)
    help_obj.delete()
    messages.success(request,'Help Record is deleted successfully.')
    return redirect('view_need_help')

def send_diagnosis_report(request):
    if request.method == "POST":
        h = request.POST['hospital_doctor']
        ho = request.POST['hospital']
        d = request.POST['doctor']
        di = request.POST['diagnosis']
        tt = request.POST['test_type']
        pr = request.POST['price']
        file = request.FILES['file']
        desc = request.POST['description']
        patient = Patient.objects.get(user=request.user)
        dia = Diagnosis.objects.get(id=di)
        if h == "Hospital":
            hospital = Hospital.objects.get(id=ho)
            obj = Diagnosis_Report.objects.create(patient=patient, test_type=tt, price=pr, diagnosis=dia, file=file, description=desc, hospital=hospital)
        else:
            doctor = Doctor.objects.get(id=d)
            obj = Diagnosis_Report.objects.create(patient=patient, test_type=tt, price=pr, diagnosis=dia, file=file, description=desc, doctor=doctor) 
        messages.success(request,'We have sent your report.')
        return redirect('view_diagnosis_report')
    hos_obj = Hospital.objects.all()
    doc_obj = Doctor.objects.all()
    lab = Diagnosis.objects.all()
    d = {'hospital':hos_obj, 'doctor':doc_obj, 'lab':lab}
    return render(request, 'patient/send_diagnosis_report.html',d)

def new_diagnosis_report(request):
    if request.method == "POST":
        h = request.POST['hospital_doctor']
        ho = request.POST['hospital']
        d = request.POST['doctor']
        ...
        dia = Diagnosis.objects.get(user=request.user)
        if h == "Hospital":
            hospital = Hospital.objects.get(id=ho)
            obj = Diagnosis_Report.objects.create(patient=patient, test_type=tt, price=pr, diagnosis=dia, file=file, description=desc, hospital=hospital)
        else:
    ...
    d = {'hospital':hos_obj, 'doctor':doc_obj, 'lab':lab, 'patient':patient}
    return render(request, 'diagnosis/new_diagnosis_report.html',d)

# def admin_view_diagnosis_report(request):
#     dia = Diagnosis.objects.get(user=request.user)
#     diagnosis_report = Diagnosis_Report.objects.filter(diagnosis=dia)
#     d = {'report':diagnosis_report}
#     return render(request, 'admin/view_diagnosis_report.html',d)

#admin view report
def admin_view_diagnosis_report(request):
    # data=Diagnosis.objects.all()
    data = Diagnosis_Report.objects.all()
    d={'data':data}
    return render(request,'admin/view_diagnosis_report.html',d)

def view_diagnosis_report(request):
    try:
        dia = Diagnosis.objects.get(user=request.user)
        diagnosis_report = Diagnosis_Report.objects.filter(diagnosis=dia)
        d = {'report': diagnosis_report}
        return render(request, 'diagnosis/view_diagnosis_report.html', d)
    except:
        pass
    try:
        doc = Doctor.objects.get(user=request.user)
        diagnosis_report = Diagnosis_Report.objects.filter(doctor=doc)
        d = {'report': diagnosis_report}
        return render(request, 'doctor/view_diagnosis_report.html', d)
    except:
        pass
    try:
        pat = Patient.objects.get(user=request.user)
        diagnosis_report = Diagnosis_Report.objects.filter(patient=pat)
        d = {'report': diagnosis_report}
        return render(request, 'patient/view_diagnosis_report.html', d)
    except:
        pass

    try:
        hos = Hospital.objects.get(user=request.user)
        diagnosis_report = Diagnosis_Report.objects.filter(hospital=hos)
        d = {'report': diagnosis_report}
        return render(request, 'hospital/view_diagnosis_report.html', d)
    except:
        pass



def dignostic_invoices(request,pid):
    # if not access(request.user):
    #     messages.success(request,'Update Your Profile and Wait for Verification')
    #     return redirect('medical_profile')
    pat = Appointment.objects.get(id=pid)
    ...
    total = str(med.aggregate(Sum('price')))[14:][:-1]
    d = {'pat':pat,'med':med,'total':total}
    return render(request,'diagnosis/invoices.html',d)
...
def accept_help(request, pid):
    help_obj = Help_Record.objects.get(id=pid)
    doctor = Doctor.objects.get(user=request.user)
    if help_obj.status == "Pending":
        help_obj.status = "Accepted"
        help_obj.helper_doctor = doctor
    else:
        help_obj.status = "Pending"
        help_obj.helper_doctor = doctor
    help_obj.save()
    messages.success(request, 'Status Changed')
    return redirect('new_need_help')

def doctor_profile(request, pid):
    data = Doctor.objects.get(id=pid)
    d = {'data':data}
    return render(request, 'doctor-profile/doctor-profile.html', d)